...
const ByCar: FC<Props> = ({ data, car, section } ) => {
	...
	const handleClick = () => {
		...
		router.push(`/catalog/${ section }/${ link.split(' ').join('-') }${ section === Section.Tires ? paramsTires : paramsDisks }`);
	...
	return (
		<>
			...
			<div className='mt-4'>
				<Button onPress={ handleClick } size='md' className='w-full bg-black uppercase' >
					{ t('choose') }
				</Button>
			</div>
		</>
